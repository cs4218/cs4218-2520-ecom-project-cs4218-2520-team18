 ...
 *     2. Seed the database with stress test data:
 *        node stress-tests/seedStressTestData.js
 *     3. Start the server: npm run server
 *
 * Usage:
 *     k6 run stress-tests/searchProductStressTest.js
 *
 * With custom options:
 *     k6 run --env BASE_URL=http://localhost:6060 stress-tests/searchProductStressTest.js
 *
 * Load Profile:
...
export function handleSummary(data) {
    ...
    const thresholdsPassed = !data.thresholds ||
        Object.values(data.thresholds).every(t => t.ok);
